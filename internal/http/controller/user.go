...
import (
	...
	"project-struct/internal/http/request"
...
func (r *UserController) GetUsersHandler(c *fiber.Ctx) error {
	...
	if err != nil {
		return c.Status(fiber.StatusInternalServerError).JSON(helper.SetHttpError(err.Error()))
	}
...
func (r *UserController) CreateUserHandler(c *fiber.Ctx) error {
	...
	if err := c.BodyParser(&body); err != nil {
		return c.Status(fiber.StatusUnprocessableEntity).JSON(helper.SetHttpError(err.Error()))
	}
	if errors := helper.ValidateSchema(body); errors != nil {
		return c.Status(fiber.StatusUnprocessableEntity).JSON(errors)
	}
	if user, err := r.Repo.CreateUser(&body); err != nil {
		return c.Status(fiber.StatusBadRequest).JSON(helper.SetHttpError(err.Error()))
	} else {
...
func (r *UserController) GetUserHandler(c *fiber.Ctx) error {
	if id, err := c.ParamsInt("id"); err != nil {
		...
		if err != nil {
			return c.Status(fiber.StatusNotFound).JSON(helper.SetHttpError(err.Error()))
		}
